...
export function unique<T>(arr: readonly T[], equal: (a: T, b: T) => boolean) {
	const res: T[] = [];
	for (let a of arr) {
...
export function transposeEmpty<T, U>(array: readonly T[][], empty: U): (T | U)[][] {
...
export function transpose<T>(array: readonly T[][]): T[][] {
	return range(Math.max(...array.map(a => a.length))).map(i => (
 ...
 */
export function interleave<T>(...arrays: readonly T[][]): T[] {
 ...
 */
export function tuplesCyclical<T>(array: readonly T[], length: number): T[][] {
 ...
 */
export function tuples<T, N extends number>(array: readonly T[], length: N): Tuple<T, N>[] {
